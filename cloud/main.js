...
Parse.Cloud.define('welcomeSender', function(request, status) {
    ...
    function getHtml(d) { //generates html for single email
        ...
        function even() {    return i%2 === 0;    }; //eo even
        ...
        function suffix() {     html += '</div>';    }; //eo suffix
        mode = mode ? mode : 0;
        gray();
        prefix();
        if(mode === 1) {
           ...
           // success('123:' + index + organization + batch + id);
            ...
            var custom = {
                ...
                "vars": [
                    ...
                    {   "name": "customheading",    "content": "<b>Welcome to Parent Planet</b>"    },
            ...
            var body = {
                ...
                "message": {
                    ...
                    "to": _to,
                    "headers": {    "Reply-To": "no-reply@parentplanet.com"    },
                    "important": false,
                    "track_opens": null,
                    "track_clicks": null,
                    "auto_text": null,
...
Parse.Cloud.define("emailSender", function(request, status) {
    ...
    function success(str) { //general success handler, returned data from previous/current promise written to console, see notes
        ...
        if(!str) {
            str = '';
            for (var i=0; i < len; i++) {    str += arguments[i]+' ';    }
        }
    ...
    function getHtml(recipient) { //generates html for single email
        ...
        function suffix() {     html += '</div>';    } //eo suffix
        function header() {
            if (section === "event") {    add(null, 'Schedule Events', headerStyle);    }
            if (section === "message") {    add(null, 'Messages', headerStyle);    }
            if (section === "homework") {    add(null, 'Homework', headerStyle);    }
        } //eo header
        function snippet() {
            ...
            function event() { //{"allDay":true,"end":"Thurs","location":"617 Memak Road","note":"too fun","repeat":"monthly","start":"Wed","title":"Test 1"}
                ...
                add('Notes', d.note)
                checkData('html', d);
                if(isRepeating && _repeat) {    add('Repeats', _repeat);    }
                suffix();
            } //eo event
    ...
    function getGroups(results) { //the organizations object from the previous step is in the closure
        ...
        query.find({success: groupFound, error: error}); //find all the organisations
    ...
    function getOrganizations(results) { //the organizations object from the previous step is in the closure
        ...
        //query.limit = 1000;
        query.limit(MAXCOUNT);
        query.find({success: found, error: error}); //find all the organisations
        return promise; //return promise, when resolved to to next step
    } //eo getOrganizations
    function sendEmails(results) { //with the emails array pass along to mandrill and send out
           ...
           //
            ...
            for (var p in batch) {
                ...
                var custom = {
                    ...
                    "vars": [
                        {    "name": "custom",    "content": html    },
                        {    "name": "customheading",    "content": "<b>Update from " + organization.name + "</b>" }, //TODO: refactor to use: Update from OrganizationName instead of default    },
                        {    "name": "footer",    "content": footer    }
                    ]
                };
                merge_vars.push(custom);
        ...
        function send(promise) { //here is where an email is sent out from Mandrill using the passed in data
            ...
            } catch(err) {    success('oops error sendEmails: '+err);    }
